...
fn compress_abracadabra_complete_pipeline_achieves_target_compression() {
    ...
    println!("Input: \"{}\"", input);
    println!("Original size: {} bytes ({} bits)", input.len(), result.original_bits);
    println!();
    
    println!("Step 1: Frequency Analysis");
    for (&byte, &count) in &result.frequency_map {
        println!("  '{}' (byte {}): {} occurrences", byte as char, byte, count);
    }
    println!();
    
    println!("Step 2: Huffman Tree Construction");
    println!("  Tree built successfully with {} unique symbols", result.frequency_map.len());
    println!();
    ...
    let encoded_sequence: String = input.bytes()
    ...
    println!("Compression ratio: {:.2}% ({} / {})", 
            result.compression_ratio * 100.0, result.compressed_bits, result.original_bits);
    println!("Space saved: {} bits ({:.1}%)", 
    ...
    assert_eq!(
        ...
        "Original size should be {} bits", expected_original_bits
    );
    ...
    for &ch in &expected_chars {
        assert!(
            ...
            "Frequency map should contain character '{}'", ch
        );
    }
    
    // 3. Verify specific frequency counts for "abracadabra"
    assert_eq!(result.frequency_map[&(b'a')], 5, "Character 'a' should appear 5 times");
    assert_eq!(result.frequency_map[&(b'b')], 2, "Character 'b' should appear 2 times");
    assert_eq!(result.frequency_map[&(b'r')], 2, "Character 'r' should appear 2 times");
    assert_eq!(result.frequency_map[&(b'c')], 1, "Character 'c' should appear 1 time");
    assert_eq!(result.frequency_map[&(b'd')], 1, "Character 'd' should appear 1 time");
    
    // 4. Verify Huffman codes were generated for all characters
    for &ch in &expected_chars {
        assert!(
            result.huffman_codes.contains_key(&(ch as u8)),
            "Huffman codes should contain character '{}'", ch
        );
        
        let code = &result.huffman_codes[&(ch as u8)];
        assert!(
            !code.is_empty() && code.chars().all(|c| c == '0' || c == '1'),
            "Huffman code for '{}' should be a non-empty binary string, got '{}'", ch, code
        );
    }
    ...
    assert!(
        ...
        max_acceptable_total_bits, result.compressed_bits
    );
    ...
    assert!(
        ...
        target_data_encoding_bits, data_encoding_bits
    );
    ...
    println!("   • Tree serialization: {} bits", result.serialized_tree.len());
    println!("   • Data encoding: {} bits", calculate_data_encoding_bits(&result.huffman_codes, input));
    println!("   • Total compressed: {} bits", result.compressed_bits);
    println!("   • Ratio: {:.1}%", result.compression_ratio * 100.0);
    println!("   • Saved: {} bits ({:.1}%)", 
             result.original_bits - result.compressed_bits,
             (1.0 - result.compression_ratio) * 100.0);
    println!("   • Target data encoding achieved: {} ≤ {} ✓", 
